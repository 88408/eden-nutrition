...
methods = """
    ...
    public PageVO<AdminSeckillVO> getAdminPage(AdminSeckillQueryDTO queryDTO) {
        ...
        List<AdminSeckillVO> list = seckillMapper.selectAdminPage(queryDTO.getProductId(), queryDTO.getStatus(), offset, queryDTO.getPageSize());
    ...
    public void deleteAdminSeckill(Long id) {
        // delete from db (assume mapping has delete or we update status, but usually delete by id)
        // seckillProductMapper doesn't have delete, let's use status update
